...
This is the starting point to run the Subaru_frontend perscription. From here, you can turn on/off toggles, change AO
settings, view different planes, and make other changes to running the perscription without changing the base
perscription or the default mm_params themselves.

This file is meant to 'cleanup' a lot of the functionality between having defaults saved in the mm_params file and
the perscription. Most changes necessary to run multiple instances of the base percription will be made here. This is
the same layot/format as the Example files that Rupert was using in v1.0. These are now meant to be paired with a more
specific percription than the original optics_propagate, which had more toggles and less sophisiticated optical train.
